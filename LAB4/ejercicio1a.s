    ...
    .word   timer_isr+1     // 28: IRQ 12: TIMER0 service routine ESTAS UTILIZAMOS
...
reset:
    ...
    LDR R1,=TIMER0_BASE
    MOV R0,#0x00
    STR R0,[R1,#CTCR] //PONEMOS UN CERO EN CTCR, DEFINIMOS CLOCK EXTERNO

    // Prescaler de 9.500.000 para una frecuencia de 10 Hz, TRABAJA A 95M CON EL PREESCALER CONSEGUIMOS FRECUENCIA 10HZ - 10 cuentas por segundo
    LDR R0,=9500000
    STR R0,[R1,#PR]

    // El valor del semperiodo para 1 Hz - INDICA que cuando se cuente 5 DEBE GENERAR INTERRUPCION
    LDR R0,=5
    STR R0,[R1,#MR3] //Podria ser el match 0,1,2,3

    // El registro de match 3 provoca reset del contador e interrupcion
    MOV R0,#(MR3R | MR3I) //PROVOCO RESET Y INTERRUPCION
    STR R0,[R1,#MCR]
    ...
    // Inicio del contador
    MOV R0,#CEN
    STR R0,[R1,#TCR] //RESETEA Y INICIA LA CUENTA
    ...
    LDR R3,=GPIO_NOT0
    MOV R0,#LED_MASK
    STR R0,[R3,#LED_GPIO << 2] //SI ESTA APAGADO LO PRENDO Y SI ESTA PRENDIDO LO APAGO
